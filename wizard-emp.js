...
function render(){
  ...
  if(step===0){
    ...
    w='<div class="qw"><div class="laststep">最後一步</div><h2 class="qh4">確認您的清洗項目</h2>'
     ...
     +'<div class="qcp"><div class="qcp-r"><span class="qcp-t">🎟️ 優惠碼</span><input id="qw-cp" class="qcp-i" type="text" inputmode="latin" autocomplete="off" spellcheck="false" value="'+_esc(_userCode)+'" oninput="__qw.setCode(this.value)"></div><div class="qcp-n">選填；完成後自動套用，也可以等到購物車下方再輸入</div></div>'
...
/* ===== 員工頁:一律標準方案,不給選 =====
   ...
   (1) 舊版這裡是「偵測到員工券就把早鳥反灰」——那是員工用正式頁時的設計。
       改成獨立員工頁後,這頁根本沒有方案概念,但它還是把 __qsPlan 設成 'std',
       害結帳彈窗冒出「您選擇的方案 標準方案·95折 [變更]」,按變更還會跑出早鳥。
   (2) 但反過來把 __qsPlan 清成 null 也不行 ——
       結帳按鈕會檢查「有沒有選方案」,清掉之後按鈕卡在「請先選擇到府方案」,
       員工根本結不了帳。
   正解:內部固定 std(員工就是兩週內到府),幫他把卡片選好讓按鈕解鎖,
        再把整個方案選擇區(#qs-plan-wrap)藏起來,並移掉寫著折數的摘要條。 */
