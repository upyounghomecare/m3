...
function render(){
  ...
  if(step===0){
    ...
    w='<div class="qw">'+stepBar()+'<h2>您要清洗的是什麼品牌？</h2><p class="sub">選擇後會顯示對應的服務項目與價格</p>'
     ...
     +(brand==='other'?'<div class="optnote">其他品牌採<b>對外牌價</b>上架，結帳輸入員工優惠碼後折 <b>75 折</b>。</div>':'')
     +(brand==='mhi'?'<div class="optnote">三菱重工為<b>原廠認證清洗</b>，結帳輸入員工優惠碼後折 <b>75 折</b>。</div>':'')
    ...
    w='<div class="qw"><div class="laststep">最後一步</div><h2 class="qh4">確認您的清洗項目</h2>'
     ...
     +'<div class="qw-sr qw-sre"><span class="qw-srn">輸入員工碼後（75 折）</span><span class="qw-srp">'+money(_emp)+'</span></div></div>'
     +'<div class="qw-empn"><b>🔑 別忘了在結帳頁輸入員工專屬優惠碼</b><br>'
     +'碼是 <b>UPE75</b> 開頭的 10 碼，<b>每組只能用一次</b>，用過就作廢。<br>'
     +'還沒拿到碼請洽客服或人資。</div>'
...
function empPlanLock(){try{
  if(window.__qsAdding||_corrBusy()||_cpBusy())return;
  if(!_empCoupon())return;
  if(window.__qsPlan!=='std'){window.__qsPlan='std';}
  /* 方案選擇彈窗裡把早鳥擋掉並說明 */
  var box=document.getElementById('qs-planbox');
  if(box){
    var cards=[].slice.call(box.querySelectorAll('.qs-plan'));
    cards.forEach(function(c){
      if(c.getAttribute('data-plan')!=='early')return;
      if(c.getAttribute('data-emplock')==='1')return;
      c.setAttribute('data-emplock','1');
      c.style.opacity='.42';c.style.pointerEvents='none';c.style.position='relative';
      var tip=document.createElement('div');
      tip.style.cssText='position:absolute;left:8px;right:8px;bottom:8px;background:rgba(4,44,83,.92);color:#fff;'
        +'border-radius:8px;padding:7px 9px;font-size:11.5px;font-weight:800;line-height:1.5;text-align:center';
      tip.textContent='員工價僅適用標準方案';
      c.appendChild(tip);
    });
  }
}catch(e){}}
...
/* ⚠️ 員工頁兩種品牌都賣,兩組名稱都要列進來,
   否則「場勘與清洗互斥」「方案記憶」「加購品守門」這些機制看不到非三菱的品項。 */
var INNAMES=['家用壁掛清洗保養','吊隱式小清洗保養','吊隱式大清洗保養','吊隱式全清洗保養','迷你四方吹清洗保養','四方吹清洗保養',
 '其他品牌 分離式室內機清洗','其他品牌 吊隱式大保養清洗'];
...
var OUTNAMES=['1對1室外機清洗','1對多室外機清洗','其他品牌 室外機清洗'];
...
function _codeOff(code){
  ...
  if(/^UPE75/.test(code))return 0.25;/* 員工價 75折(2026-08-25,全站最優惠,僅適用標準方案) */
