...
var P={
 ...
 air:{pid:'28je90BY1yYVlQm91Mqp6lDN',price:1600,img:imj('LWx9yMqPlG7b699ZlEpb1wO7')},
 ...
 dh:{price:21900,oprice:25900,img:imj('GBnwbKdx3dvWpwwDNW7954ej')},
...
+''
+'.qw .qh4{font-weight:900;-webkit-text-stroke:.5px currentColor;margin-top:8px}'
+''
+''
+''
...
+''
+''
+'.qw .qplans{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:0}'
...
function _qwFoot(card){try{
 ...
 /* 2026-09-23 優惠碼欄位要跟按鈕一起黏在底部:否則會被黏底的按鈕列蓋掉一半(老闆實機截圖抓到) */
 ...
 if(cp)f.appendChild(cp);
 f.appendChild(nav);
 /* 貼齊視窗最底緣:讀精靈自己的下內距來抵,不寫死數字 */
 var pb=getComputedStyle(card).paddingBottom;
 f.style.bottom='-'+pb;f.style.paddingBottom=pb;
 /* 2026-09-23 黏底區會蓋住它上面的內容(老闆兩次實機截圖抓到:先是優惠碼小字、後是「先看服務說明」被切一半)。
    讓內容區底部空出「黏底區高度」,捲到底時上面的東西就完全露得出來。高度隨文字換行變動,所以用量的、不寫死。 */
 var _pad=function(){try{
...
function render(){
  ...
  if(step===0){
    ...
    var _wh=_whQualQ();                     /* 全戶方案:家用 + 室內機3台以上 → 標準95折升級92折 */
    var _off={std:_wh?WH_OFF:0.05,early:0.15};
    ...
    w='<div class="qw"><div class="laststep">最後一步</div><h2 class="qh4">你想要多快安排到府清洗？</h2><p class="sub">越有彈性、折扣越多，二選一</p><div class="qplans">'+planCard('std',PLAN_STD,'安排兩週內到府服務','qpn-std')+planCard('early',PLAN_EARLY,'安排30天後到府服務','qpn-early')+'</div>'+(plan==='std'?'<div class="qpnote qpn-info">🗓️ 標準方案：<b>專人聯繫起 2 週內</b>到府清洗<br>📞 下單付款後由專人來電約時間</div>':'')+(plan==='early'?'<div class="qpnote qpn-warn">⏰ 早鳥方案：<b>專人聯繫約時起 30 天後</b>到府清洗<br>📞 下單付款後由專人來電約時間</div>':'')+'<div class="qdl" onclick="__qw.seeDetail()"><div class="qdl-ic">📖</div><div class="qdl-tx"><div class="qdl-t1">先看服務說明/規範完整圖文介紹</div><div class="qdl-t2">服務內容、清洗流程、施工實例</div></div><div class="qdl-ar">›</div></div>'+'<div class="qcp"><div class="qcp-r"><span class="qcp-t">🎟️ 優惠碼</span><input id="qw-cp" class="qcp-i" type="text" inputmode="latin" autocomplete="off" spellcheck="false" value="'+_esc(_userCode)+'" oninput="__qw.setCode(this.value)"></div><div class="qcp-n">選填；完成後自動套用，系統會採用<b>折扣較多</b>的那一張</div></div>'+'<div class="nav"><button class="btn gho" onclick="__qw.go(3)">上一步</button><button class="btn pri" '+(plan?'':'disabled')+' onclick="__qw.confirmPlan()">完成，前往結帳</button></div></div>';
  }
...
function planMemoryWatch(){try{
  ...
  else if(nm.indexOf('標準95折')>=0||nm.indexOf('全戶92折')>=0)p='std';/* 全戶92折同屬標準線 */
  if(!p)p=_planLoad();
...
/* 結帳彈窗的方案卡片文字。達標時「標準方案·95折」要改寫成「全戶方案·92折」,
   否則客戶會看到彈窗寫 95 折、購物車卻折了 8%,以為系統算錯。
  ...
  if(collapsed){
    ...
    wrap.innerHTML='<div style="font-size:11.5px;color:#8a6a1f;font-weight:700;letter-spacing:.3px;margin:0 0 6px">您選擇的方案</div>'
      ...
      /* 2026-09-14 老闆要求移除底下那句提示。
         原本 PLAN_CHG_OFF=1 時會寫「想換方案？請關掉此視窗，用上方的引導精靈重新挑選」——
         那是 08-31 停用「變更」鈕時補的說明,老闆看實機後認為多餘。
         ⚠️ PLAN_CHG_OFF 改回 0(恢復變更鈕)時,下面那句「選錯了?點變更」要一起回來。 */
      +(PLAN_CHG_OFF?'':'<div style="font-size:11px;color:#9aa7b4;margin-top:5px;padding-left:2px">選錯了?點「變更」可調整方案</div>');
    var c=wrap.querySelector('.qsps-chg');if(c)c.onclick=function(){_renderPlanSum(wrap,false);};
  }else{
    var card=function(k){var info=_planInfo(k);var sel=(k===p);return '<div class="qsps-pick" data-p="'+k+'" style="flex:1;border:'+(sel?'2.5px solid #B8860B':'2px solid #d3dde9')+';border-radius:11px;overflow:hidden;cursor:pointer;position:relative;box-shadow:0 2px 6px rgba(4,44,83,.08)"><img src="'+_PLANB+info.img+'" style="width:100%;display:block">'+(sel?'<div style="position:absolute;top:6px;right:6px;background:#B8860B;color:#fff;font-size:10px;font-weight:800;border-radius:999px;padding:2px 9px">已選</div>':'')+'</div>';};
...
/* ⚠️ 曾經在這裡寫過 stripCouponForSurvey():場勘單若被套上方案券就自動移除。
   ...
   後來也試過「購物車有東西就不給場勘入口」,一樣沒用(券是加入商品之後才掛上來的)。

   2026-08-20 老闆定案:**可以接受被折,但場勘只算標準方案、折扣上限就是 95 折**。
   所以不再移除券,改成「封頂」—— 車上若是早鳥85折之類更大的折扣,就換成 UP95。
   附帶好處:客戶那組一次性專屬碼(UPR85/UPB92)不會被浪費在一張 $600 的場勘單上,
   而且在購物車換券不會消耗使用次數(已實測),留著下次買清洗還能用。 */
var SURVEY_CAP_CODE='UP95';   /* 標準方案的券 */
var SURVEY_CAP_OFF=0.05;      /* 95 折 = 折扣率 0.05,場勘單的上限 */
var _svCapTry=0,_svCapAt=0,_svCapSeen=0,_svCapKey='';
function capCouponForSurvey(){try{
  if(window.__qsAdding||_corrBusy()||_cpBusy())return;
  /* ⚠️ 這裡「不可以」用 _surveyOnly() —— 它是黏著的,購物車讀不到時會沿用上次結論。
     客戶從場勘單改成清洗單的那一瞬間若沿用了舊結論,就會把他的早鳥85折降成95折,
     客戶白白多付 $300。降級是最嚴重的一類錯,寧可少鎖一次也不能誤降。
     所以這裡要求「購物車真的讀得到,而且當下確認只有場勘」才動手。 */
  var c=_cartArr();
  if(!c.length){_svCapSeen=0;return;}
  var hasSurvey=false;
  for(var i=0;i<c.length;i++){if((c[i].ProductName||'').indexOf(SURVEY_PREFIX)===0){hasSurvey=true;break;}}
  if(!hasSurvey||(_indoorInCart()+_outdoorInCart())>0){_svCapTry=0;_svCapSeen=0;_svCapKey='';return;}
  var off=_cartOff();
  if(off<=SURVEY_CAP_OFF+0.001)return;   /* 沒券、或已經是95折(含更小) → 不用動 */
  var now=(new Date()).getTime();
  /* 購物車金額或折扣率一有變動就重新給機會;沒變動就別一直重試 */
  var key=_cartSub()+'/'+off;
  if(key!==_svCapKey){_svCapKey=key;_svCapTry=0;_svCapSeen=0;}
  /* 1SHOP 一次只吃一個購物車操作,剛變動時搶著送必失敗 —— 先靜候 1.8 秒讓它落地 */
  if(!_svCapSeen){_svCapSeen=now;return;}
  if(now-_svCapSeen<1800)return;
  if(now-_svCapAt<3000)return;
  /* ⚠️ 硬上限:試三次就停手。舊版的 stripCouponForSurvey 是因為計數器會被自己歸零,
     上限形同虛設,才演變成無限閃爍。這裡的次數只由上面的 key 變動來歸零。 */
  if(_svCapTry>=3)return;
  ...
  _svCapTry++;_svCapAt=now;
  /* 換券中間有一小段沒有任何優惠券,此時被按下結帳會用原價成立訂單 → 先鎖結帳鈕 */
...
/* ═══ 全戶方案:資格變了要換券(2026-09-09) ═══
   方案券是「完成精靈那一刻」決定的(內文JS 的自動補券只在購物車完全沒券時才動作)。
   所以客戶在購物車裡直接改台數,券不會跟著換,會出現兩種錯:
     ① 3台拿到92折 → 在購物車減成2台 → 券還是92折 → **每單少收 3%**
     ② 2台拿到95折 → 在購物車加到3台 → 券還是95折 → 客戶少拿 3%,
        而且畫面上 D1 的提示會寫「已達全戶方案 92 折」、折扣列卻寫 95 折 —— 自打嘴巴,一定客訴。
   ②比①更急:少收老闆看不到,自打嘴巴客戶當場就看到。

   做法完全比照 capCouponForSurvey(那支已經在正式頁跑了三週,是這頁唯一被驗證過的換券寫法):
   等購物車靜下來 1.8 秒才送、同一個狀態最多試 3 次、換券空窗鎖住結帳鈕、舉 _cpFixing 繞過自家的防降級。

   ⚠️ 只碰「標準95折/全戶92折」這兩張方案券。早鳥券、以及客戶自己的碼(VIP88/回購85/員工價)
      一律 return —— 誤把客戶的好券換成 95 折是最嚴重的一類錯(客戶白白多付錢)。 */
var _whTry=0,_whAt=0,_whSeen=0,_whKey='';
function whCouponGuard(){try{
  if(window.__qsAdding||_corrBusy()||_cpBusy())return;
  var c=_cartArr();
  if(!c.length){_whSeen=0;_whKey='';return;}
  var cp=null;
  for(var i=0;i<c.length;i++){if(Number(c[i].ProductType)===99){cp=String(c[i].Title||'');break;}}
  if(!cp)return;                                   /* 沒券 → 內文JS 的自動補券會處理 */
  var isWh=cp.indexOf('全戶92折')>=0, isStd=cp.indexOf('標準95折')>=0;
  if(!isWh&&!isStd){_whTry=0;_whKey='';return;}     /* 早鳥券或客戶自己的碼 → 不干涉 */
  /* 場勘單交給 capCouponForSurvey 處理,兩支不要搶同一張券 */
  var hasSv=false;
  for(var j=0;j<c.length;j++){if((c[j].ProductName||'').indexOf(SURVEY_PREFIX)===0){hasSv=true;break;}}
  if(hasSv&&(_indoorInCart()+_outdoorInCart())===0){_whTry=0;_whKey='';return;}

  var q=_whQualC();
  if(isWh===q){_whTry=0;_whSeen=0;_whKey='';return;}  /* 券跟資格一致,沒事 */

  var want=q?WH_CODE:'UP95';
  var now=(new Date()).getTime();
  var key=_indoorInCart()+'/'+_cartSub()+'/'+want;
  if(key!==_whKey){_whKey=key;_whTry=0;_whSeen=0;}
  if(!_whSeen){_whSeen=now;return;}                  /* 先靜候,1SHOP 一次只吃一個購物車操作 */
  if(now-_whSeen<1800)return;
  if(now-_whAt<3000)return;
  if(_whTry>=3)return;                               /* 硬上限,絕不變成無限閃爍 */
  var el=document.querySelector('[name="CouponNumber"]');if(!el)return;
  var btn=document.querySelector('[onclick*="submitCouponNumber"]');if(!btn)return;
  _whTry++;_whAt=now;
  window.__qsCpBusy=now;
  try{_lockCheckout(true);}catch(e){}                /* 換券中間沒有券,此時被結帳會用原價成立訂單 */
  el.value=want;
  try{el.dispatchEvent(new Event('input',{bubbles:true}));}catch(e){}
  _cpFixing=1;                                       /* 不舉旗會被自家的防降級擋掉(95折對92折是「更差」) */
  try{if(window.__qsApplyPlanCoupon){window.__qsApplyPlanCoupon(function(){});}else{btn.click();}}catch(e){}
  _cpPending=null;                                   /* 系統換的券不可以寫進「客戶的最佳券」記憶 */
  setTimeout(function(){window.__qsCpBusy=0;_cpFixing=0;_cpPending=null;},2600);
}catch(e){}}
...
/* ═══ 全戶方案(2026-09-09 老闆定案) ═══
   一般家用 + 室內機 3 台(含)以上 → 標準方案的 95折 升級成 92折。
   早鳥不設級距 —— 85折本來就比 92折低,再給就變成「洗越多越貴」的怪事。
   營業場所/重油汙不適用(那條線走商用加價,本來就另外計價)。

   ⚠️ 「3台」只算室內機,不把室外機加總。理由:
      客戶口中的「我家有3台冷氣」講的就是室內機,門檻跟客戶直覺一致,客服不用解釋;
      若加總室外機,「1台冷氣(室內+室外) + 1台單獨室外機」也會湊到3台,
      等於只有一台冷氣的客戶拿到全戶折扣,跟「全戶」這個名字對不上。
      1對多(一台室外機帶3台室內機)算3台 —— 那才是真正的全戶情境,理應給。
...
/* ═══ 全戶方案的「差一台」提示(2026-09-09 老闆選 A4 + B3) ═══
   ...
   金額也只有在這裡算得出來。首屏(C案)老闆決定不做。

   顯示規則:
     0~1 台 → 不顯示。差 2 台以上才推,客戶只會覺得被推銷。
     2 台   → 藍色,進度條 2/3,告訴他第 3 台實際只多付多少。
     3 台+  → 綠色,進度條填滿,告訴他多折了多少。
   營業場所一律不顯示(商用走加價那條線,不適用全戶)。

   ⚠️ 「第 3 台只多付 $X」的 X 必須算得準,不能拿平均價或最低價唬弄 ——
...
/* ═══ 全戶方案:購物車版提示(D1,老闆要求用跟 A4 一樣的視覺) ═══
   ...
   ⚠️ 這是純顯示,不碰購物車。這一頁所有出過包的機制都是「自動增刪購物車」造成的,
      D1 只讀不寫,插一個 div 在小計上面。 */
var _WHPROT=['三菱重工除濕機','AIRMON'];
function _whCartProt(){var c=_cartArr(),s=0;c.forEach(function(x){var nm=x.ProductName||'';
  for(var i=0;i<_WHPROT.length;i++){if(nm.indexOf(_WHPROT[i])>=0){s+=Number(x.LineTotal)||0;break;}}});return s;}
function _whCartTop(){/* 車上台數最多的室內機:單價直接用 LineTotal/Quantity,不查價目表,絕不會說一個價收另一個價 */
  var c=_cartArr(),best=null,bq=0;
  c.forEach(function(x){var nm=x.ProductName||'',q=Number(x.Quantity)||0;
    for(var i=0;i<INNAMES.length;i++){if(nm.indexOf(INNAMES[i])===0){if(q>bq){bq=q;best={name:INNAMES[i],price:Math.round((Number(x.LineTotal)||0)/(q||1))};}break;}}});
  return best;}
function whCartNote(){try{
...
function _whTopKind(){
  ...
  var best=null,bq=0;
  for(var i=0;i<INK.length;i++){var k=INK[i],q=qty[k]||0;if(q>bq){bq=q;best=k;}}
  if(!best)return null;
  var nm='';for(var j=0;j<INDOOR.length;j++){if(INDOOR[j].k===best){nm=INDOOR[j].n;break;}}
...
/* 內文JS 的 planCode() 會即時呼叫這支決定要送 UP95 還是 UP92WH。
   ...
   ⚠️ wizard.js 若載入失敗,內文JS 讀不到這支就退回 UP95(95折)。
      失敗方向是「少折給客戶」而不是「少收老闆的錢」,這是刻意選的。 */
try{window.__qsWHF=_whQualC;}catch(e){}
...
function _setCorr(M){if(_adjSyncing)return;M=Math.max(0,Math.round(M));
 ...
 (function step(){
   if(i2>=ops.length){setTimeout(function(){_adjSyncing=false;},900);return;}
   var op=ops[i2++];
   try{
...
function planMoneyGuard(){try{
  ...
  var real=(_cpTitle.indexOf('早鳥85折')>=0)?'early':((_cpTitle.indexOf('標準95折')>=0||_cpTitle.indexOf('全戶92折')>=0)?'std':null);/* 全戶92折是標準線的級距,方案身分仍是 std */
...
function bindCouponGuard(){try{
  ...
  window.submitCouponNumber=function(a){
    try{
      ...
      /* ⚠️ 2026-09-10 加:UP92WH 是「系統方案券」,不是給客戶輸入的碼。
         它在後台是公開券、沒有最低金額限制,所以只要有人知道代碼,1 台也能套上去。
         原本只靠 whCouponGuard 事後換回來 —— 但那是「先讓他套上、再改回去」,
         中間有幾秒鐘他真的拿著 92 折,期間結帳就會少收(1~2 台約少收 $90~150)。
...
/* ===== 2026-09-22 詳情頁「目錄＋段落收展」=====
   ...
   老闆定案:分段照下表;2026-09-22 晚改為 6 段全部預設展開(客戶仍可自己點標題收合)。
   ⚠️ 只「搬動」原本的 <img>,不改 src、不改 lazy loading(圖片共 12MB,不可全部提早載入)。
   ⚠️ 找不到詳情圖(例如換了詳情圖檔名)就什麼都不做,頁面維持原樣。 */
/* 常見問題文字版(2026-09-22):內容逐字取自「客服 FAQ 題庫」Google Sheet 的「常見問題」分頁,
   分類與順序照原本 17/18/19 三張常見問題圖(A~F 74 題),老闆同意再加 3 題(A 類 2 題、D 類 1 題);2026-09-22 Sheet 移除 ATM 付款相關內容(刪 1 題、改 3 題);同日新增「如何預約服務？」,共 77 題。⚠️ 對客戶的說法只能以那張 Sheet 為準,改 Sheet 後要同步更新這裡。 */
var QD_FAQ=[["A","預約、付款與發票",[["早鳥方案與標準方案有什麼不同？怎麼選？","依您的需求，可選擇兩種方案：\n\n早鳥方案（提早規劃、省錢）\n・全室內外機清洗服務品項享 85 折\n・自專人約時起，需安排 30 天後（含）到府；淡旺季皆可\n・適合：不趕時間、想省錢的您\n\n標準方案（近期就要洗）\n・標準價格\n・安排 2 週內到府\n・適合：想盡快清洗的您\n\n兩方案的取消／改約規則不同（早鳥約定後即依規則計費；標準享較彈性的免費期限），詳見「訂單取消／改約須知」。"],["要如何購買冷氣清洗服務？","您可以點選下方的【清洗購買】按鈕，依方便的付款方式選擇購買管道：\n\n1️⃣ 1SHOP 線上商城\n支援信用卡付款。\n2️⃣ LINE 線上商城\n支援 LINE Pay 付款。\n\n完成下單及付款後，將由專人主動與您聯繫，協助安排到府服務時間。\n如有任何問題，也歡迎隨時透過 LINE OA 詢問我們。"],["如何預約服務？","從下單到完工，共五個步驟：\n\n1️⃣ 購買服務\n選擇規格、加入購物車完成結帳。\n2️⃣ 預約服務時間\n下單後 1 個工作天內，將有專人主動與您聯繫、安排到府時間。\n3️⃣ 到府服務\n服務前一天以 Email 提醒，技師準時到府清洗。\n4️⃣ 服務驗收\n清洗完成後，當場與技師一同檢視清潔成果、確認驗收。\n5️⃣ 滿意度問卷\n服務完成後 1 天發送滿意度問卷，歡迎回饋您的寶貴意見。\n\n如有任何疑問，也歡迎隨時透過 LINE OA 聯繫客服。"],["可以服務完成後再付款嗎？","目前清洗服務皆採先付款、後安排服務，恕無法於服務完成後再付款。\n\n您可透過以下方式完成下單及付款：\n1️⃣ LINE線上商城\n付款方式：LINE Pay\n2️⃣ 1SHOP線上商城\n付款方式：信用卡\n\n完成付款並確認款項後，我們才會依訂單順序協助安排到府服務。\n📌 若技師到府後臨時增加清洗項目，且當天後續班表時間允許，加洗費用可採現場現金付款。"],["發票如何開立？","本服務使用「綠界電子發票」，發票開立方式及時間依下單管道如下：\n\n1️⃣ LINE 線上商城下單\n完成下單付款後，將於 1個工作天內完成發票開立。\n2️⃣ 1SHOP 線上商城下單\n完成下單付款後，系統將自動開立發票。\n\n📌 發票類型說明：\n如需開立「公司統編發票」，將開立為紙本發票，請於下單時正確填寫「公司抬頭」及「統一編號」。\n非統編發票則預設開立為電子發票。\n發票開立完成後，綠界將寄送「發票開立通知」至訂單填寫或您提供的 Email，請留意收件匣及垃圾郵件匣。\n如需使用「個人手機載具」，請於下單時填寫正確的手機條碼載具。\n\n🧾 電子發票如需列印紙本：\n可於收到綠界發票開立通知信後，依信件提供的發票資訊，前往7-ELEVEN ibon操作列印。\nhttps://www.ecpay.com.tw/Business/invoice_KIOSK_General"],["下單後多久會接到聯絡？","通常在您完成下單並付款完成後的 1～3 個工作天內，客服人員會主動與您聯繫，確認服務時間與相關細節。"],["訂購後需要在多久內完成預約？","為利技師排程，訂購後需於 30 日內與約時人員約定「到府清洗保養服務日期」。\n若逾期仍未預約，且經多次不同時段致電、留言仍無法取得聯繫，我們將取消訂單並依原付款方式辦理退款，敬請留意。"],["如果一直聯繫不上我，訂單會怎麼處理？","完成訂購後，將由專人於 0.5-1 個工作天內與您聯繫約時。\n若經 3 次以上、於不同時段致電並留言仍無法取得聯繫，訂單將暫緩安排。\n若持續無法聯繫，將取消訂單並依原付款方式辦理退款。建議您留意來電，或直接透過 LINE 客服回覆，以利盡快安排。"],["如何查詢我的冷氣機齡？","請檢視室內機側面的產品資訊標籤，可找到「製造年份」或「出廠日期」。"],["從哪裡可看到機型／機號？","位於室內機右側／下方，或室外機右側的銘板貼紙上。"],["可以指定時段或假日施工嗎？","可預約時段為週一至週日（全年無休），服務時間 09:00–18:00。實際到府時間以客服協調排程為準。"],["你們服務哪些地區？偏遠地區要加價嗎？","服務範圍請見詳情頁「服務區域一覽」。部分偏遠地區需加價 NT$600，另有少數未開放區域；若不確定您的地址是否可服務，歡迎預約前先透過 LINE 客服確認。"],["客服會主動通知我操作 ATM、儲值嗎？（防詐）","不會。本服務客服絕不會主動通知「誤扣款／欠費／分期設定錯誤」等理由，要求您操作 ATM、網路銀行或超商儲值。如接獲可疑來電或訊息，請勿依指示操作，並可撥打 165 反詐騙專線查證。"],["有服務其他品牌的冷氣嗎？","本服務主要提供三菱重工原廠空調的清洗保養。若為其他品牌機型，建議您可洽詢其他品牌原廠服務單位或原購買經銷商，協助確認相關清洗與保養費用，感謝您。"],["技師遲到或未到怎麼辦？","技師會依約定時段到府。若因交通、前一場次延誤等因素可能遲到，客服或技師會主動提前通知您並協調；若遇天災等不可抗力需改期，亦會主動為您重新安排。如已到約定時間卻遲未接到聯繫，歡迎直接透過 LINE 客服或客服專線與我們確認，我們會立即為您查詢。"]]],["B","服務內容、範圍與施工方式",[["壁掛式/吊隱式/四方吹空調清洗是否等同冷氣維修？","不是。清洗屬保養與衛生清潔服務，並非故障修復；若冷氣本身已有異音、不冷、漏冷媒、跳電等狀況，需另行安排檢修。"],["冷氣清洗後還需要補冷媒嗎？","冷媒不是消耗品，通常不會因清洗而需要補充；若原本就有不冷、結霜、結冰等情形，建議另外安排檢測。"],["室外機是如何清洗的？","現場將由技師使用高壓清洗設備進行作業，主要清除室外機外殼表面的灰塵，以及散熱鰭片上的污垢。室外機清洗採不拆卸、不移動原安裝位置的方式進行，是否能施作仍需依現場環境條件評估後確認。\n\n＊如室外機現場有以下情況，將無法提供清洗服務：\n1. 室外機設置於外牆懸掛、無法安全接近，或有工安疑慮的環境\n2. 清洗產生的污水排放可能影響其他住戶\n3. 散熱鰭片老化嚴重，不適合再進行清洗"],["可以只清洗室外機嗎？","可以。室外機可單獨清洗，也可搭配室內機一起清洗。 ・單獨清洗且只洗 1 台：需加收車馬費 NT$600。 ・清洗 2 台以上，或室外機搭配室內機一起清洗（不限台數）：免車馬費。 詳細清洗費與商用加價，請見下方「室外機清洗的費用怎麼算？」。"],["為什麼室外機需要清洗？一定要一起清洗嗎？","定期清潔與保養室外機，有助於降低耗電並減少壓縮機異常的風險。若室外機長時間累積灰塵與髒污，可能影響散熱效率，進而讓壓縮機運轉負擔加重，影響整體機器效能。\n\n室外機一定要一起清洗嗎？不一定，但若室外機散熱片髒污明顯、周邊環境粉塵較多或使用年限較長，建議可一併評估清洗，以維持較佳散熱效率。"],["冷氣清洗大約需要多久時間？","實際作業時間會依機型、數量、髒污程度、天花高度、維修孔大小與現場施工難度而有所不同。單台預估清洗／保養時間參考如下： 【室內機】 ・家用壁掛式：約 1.5~2 小時／台 ・吊隱式：約 1.5~2.5 小時／台 ・四方吹（嵌入式）：約 1.5~2.5 小時／台 【室外機】 ・1 對 1：約 0.5 小時／台 ・1 對多（多聯）：約 0.5~1 小時／台 ＊以上為一般家用環境之預估；若為商用（營業場所）或重油污環境，將視現場狀況延長清洗保養時間。 ＊實際到府時間仍以客服協調排程為準。"],["冷氣清洗服務是現場施作還是拆回處理？","本服務為到府現場施作，技師會依機型與現場條件進行拆卸、防護、清洗與組裝。"],["冷氣清洗服務包含哪些項目？","清洗範圍會依現場機型與可施作範圍評估（一般涵蓋濾網、蒸發器、風鼓／風輪、集水盤、排水管可及處、出風口周邊等），實際仍需依現場結構與機型評估。各機型清洗項目如下：\n\n■ 家用壁掛式室內機（採現場掛洗）\n・面板（蓋板）拆解清洗\n・機外殼拆解清洗\n・過濾網拆解清洗\n・扇葉拆解清洗\n・水盤拆解清洗\n・風鼓以不拆洗方式清洗（不拆解，清潔其可及範圍）\n\n■ 吊隱式室內機\n〔小保養清洗方案〕不含風鼓拆洗\n・排水管疏通\n・進迴風濾網拆解清洗\n・馬達潤滑上油\n・功能檢查\n〔大保養清洗方案〕\n・功能檢查\n・風鼓拆解清洗（基本含清洗兩風鼓）\n・排水管疏通\n・鰭片清洗\n・馬達潤滑上油\n・進迴風濾網清洗\n〔全拆洗方案〕\n・功能檢查\n・風鼓拆解清洗（基本含清洗兩風鼓）\n・排水管疏通\n・鰭片清洗\n・馬達潤滑上油\n・進迴風濾網清洗\n・吊隱式下蓋全拆洗\n・蒸發器鰭片清洗\n・下方蓋板全拆洗\n\n■ 四方吹（嵌入式）室內機\n〔小保養清洗方案〕\n・排水管疏通\n・進迴風濾網拆解清洗\n・馬達潤滑上油\n・功能檢查\n〔大保養清洗方案〕\n・功能檢查\n・風鼓拆解清洗（基本含清洗兩風鼓）\n・排水管疏通\n・鰭片清洗\n・馬達潤滑上油\n・進迴風濾網清洗"],["有包含室外機清洗嗎？","室外機清洗需另行加購。建議一併清洗可提升整體散熱效率與冷房效果，並有助於節省電力、延長機器壽命。"],["清洗後多久可以正常開冷氣使用？","清洗完成並經技師測試確認功能正常後，即可正常使用。\n建議清洗後先以送風或除濕模式運轉 30～60 分鐘，待內部充分乾燥後再切換至冷氣模式，可獲得更佳的使用體驗。"],["清洗時會用到什麼清潔劑？會不會對人體或環境有害？","清洗時會使用中性或弱鹼性的冷氣專用清潔劑，具環保與可生物分解特性，無毒且安全。技師在作業時也會做好防護措施，確保藥劑不會殘留於機體或對環境造成影響。"],["冷氣清洗會不會造成損壞？","專業技師會依照標準作業流程，使用專用工具與環保清潔劑，確保不會損及冷氣內部零件。若冷氣機本身老舊或已有潛在故障，技師會於清洗前進行檢視並事先告知您相關風險。"],["清洗完成後要怎麼驗收？","清洗完成後，技師會與您當場一同驗收確認清潔成果；若有加洗項目，請確認服務驗收單已備註加洗項目與金額，以保障您的權益。"],["漏水問題清洗後可以解決嗎？","不一定。若漏水是因濾網阻塞、集水盤髒污或排水管輕微堵塞造成，清洗後通常會改善；但若漏水源自排水管老化、坡度不良、嚴重堵塞、管線破損或安裝問題，屬結構性問題，清洗無法根治，需另行處理或維修。技師會於現場評估漏水原因並向您說明。"]]],["C","費用、加購與取消規則",[["現場可以臨時加洗嗎？","技師到府服務後，如有臨時加洗需求，可直接向技師提出。\n技師會依當天後續服務班表及現場作業時間評估；如時間允許，即可現場加洗。加洗費用依賣場公告價格為準，並採現場現金付款。\n技師收款後，會將加洗項目及金額記錄於「服務驗收單」中。\n\n📌 加洗項目發票說明\n加洗項目的發票，將依原主訂單留存的發票資訊開立，並於服務完成後 1～3個工作天內完成開立。發票開立後，綠界將寄送「發票開立通知」至主訂單留存的 Email。"],["日期確認後想改約或取消，規費是多少？","正式約定服務日期前，兩方案皆可免費取消／改約。服務日期一經約定，取消／改約補償費（每人）依方案不同：\n【標準方案】\n· 服務日前 5 個工作天 17:00 前：免費\n· 服務日前 5 個工作天 17:00 後：NT$300\n· 服務日前 4 個工作天 17:00 後：NT$600\n· 服務日前 1 個工作天 17:00 後或當日：NT$1,000\n【早鳥方案】（享早鳥優惠價，約定後即依下列計費）\n· 約定後至服務前：一律 NT$600\n· 服務日前 1 個工作天 17:00 後或當日：NT$1,000"],["技師到府後發現無法服務，會收費嗎？","視情況而定。若是因我方技術問題（例如特殊機型無法拆解），不收任何費用。\n‧ 若技師到府後發現設備已有故障無法清洗，則酌收車馬費 NT$600。\n‧ 若因現場環境限制、訂購規格不符、非三菱重工空調品牌(如三菱電機)、或事先未告知為餐飲店家或油煙環境，同樣酌收車馬費 NT$600 （若服務前已收款，將扣除車馬費後退款）。"],["技師到府後聯絡不到我，怎麼算？","約定時間後超過 30 分鐘後仍無法聯繫上您，比照「服務當日取消」標準，酌收 NT$1,000／人。"],["購買多台服務，若其中有台設備當天無法清洗，費用如何處理？","若技師到府後，因現場環境限制、設備故障或其他非人為因素，導致部分設備無法施作，將依無法施作的台數按比例退還對應費用，已完成清洗的台數費用則維持不變。\n＊請注意，若訂單享有折扣優惠，退款金額將以折扣後的實際付款金額計算，而非定價。\n\n＊舉例來說，若您購買 3 台壁掛式空調深度清洗（定價每台 NT$3,000，享 9 折優惠後每台 NT$2,700，共付 NT$8,100），\n到府後技師發現其中 1 台設備故障或因其他因素而無法施作，則退還折扣後金額 NT$2,700，實際收取 NT$5,400。\n\n退款作業將由客服與您確認後，依原付款方式辦理，如有疑問歡迎聯繫我們。"],["現場可以加洗嗎？如何收費？發票如何開立？","可以先請技師現場評估，但是否能加洗，需依當日排程、設備狀況及可服務時間確認。\n\n💰 加洗費用參考\n・家用壁掛室內機：NT$3,000／台\n・室外機 1對1：NT$1,000／台\n・室外機 1對多：NT$1,500／台\n\n✅ 加洗前會先確認項目與費用，經您同意後才會施作。\n💵 現場加洗費用以現金收取，發票將依主訂單發票資訊開立。\n📝 為保障您的權益，請確認服務驗收單有備註加洗項目及收現金額。"],["若冷氣很髒、油煙很重或有厚重霉垢，會影響施工嗎？","會。若髒污程度明顯高於一般家用環境，可能增加工時、施工難度與耗材使用，必要時會於現場另行評估是否加價或加時處理"],["室外機清洗的費用怎麼算？會不會另收車馬費？","【清洗費 · 依機型計算】 ・一對一分離式：每台 NT$1,000 ・一對多（多聯機）：每台 NT$1,500 【一般家用 · 車馬費 NT$600】 僅在「單獨清洗室外機、且只洗 1 台」時加收 NT$600（技師專程往返車程成本）。 免車馬費：清洗 2 台以上室外機，或室外機搭配室內機一起清洗（不限台數）。 【營業場所（商用／重油污）· 商用加價 NT$1,000／台】 營業場所改收「商用加價」，不另收車馬費： ・室外機搭配室內機清洗 → 室外機不加價 ・單獨清洗室外機 → 每台加收 NT$1,000"],["清洗服務的價格是多少？","清洗費用依機型與清洗方案計算：\n\n【室內機】\n・家用壁掛式：NT$3,000／台\n・吊隱式（小清洗保養）：NT$1,600／台\n・吊隱式（大清洗保養）：NT$3,000／台\n・吊隱式（全清洗保養）：NT$5,000／台\n・四方吹（標準）：NT$5,000／台\n・四方吹（迷你）：NT$4,000／台\n\n【室外機】\n・一對一分離式：NT$1,000／台\n・一對多（多聯機）：NT$1,500／台\n\n【方案與優惠】\n・早鳥方案：全室內外機清洗服務品項享 85 折\n・標準方案：以標準價格計算，不定期推出 95 折等短期優惠\n・單獨清洗室外機且只洗 1 台：加收車馬費 NT$600（清洗 2 台以上或搭配室內機則免收）\n・偏遠地區：部分區域加價 NT$600"],["怎麼知道我的吊隱機型要不要加洗風鼓費用？","【方案已含 2 個風鼓】\n吊隱式的「大保養」與「全保養」清洗方案，\n基本已包含 2 個風鼓的拆洗保養。\n風鼓數 3 個以上的機型，超出的部分需要加洗。\n\n【加洗費用】\n・風鼓 1～2 個：免加洗\n・風鼓 3 個：加洗 1 個，+NT$500\n・風鼓 4 個：加洗 2 個，+NT$1,000\n\n【怎麼知道自己是幾個風鼓】\n・型號印在室內機機身、或維修孔內側的標籤上\n・找到型號後，對照『吊隱式風鼓加洗對照表』馬上就知道\n・不確定的話，把型號傳給 LINE 客服，我們幫您確認"]]],["D","施工前準備與現場條件",[["清洗會不會弄髒家裡？","技師在作業前會鋪設防水防塵布，並使用專用集水裝備，確保清洗過程中污水不外濺，盡量保持居家環境整潔。"],["若現場評估後不適合施工，會怎麼處理？","若因安全、結構、空間、維修孔、水電、排水或設備老化等因素無法施作，技師會現場說明原因，後續再依平台或賣場規範處理改期、取消或相關費用。"],["施工時會有噪音嗎？","會。拆裝、沖洗、高壓設備運作與機體測試過程中，現場可能產生正常施工聲響，建議避開需要安靜的時段。"],["若家中沒有水龍頭或排水孔該怎麼辦？","冷氣清洗通常需要使用水源與排水設施。若現場沒有可用的水龍頭或排水孔，請於預約時提前告知客服或技師，方便他們攜帶延長水管、集水桶等輔助工具，或評估現場是否具備清洗條件。"],["冷氣若裝在挑高或特殊位置，還能清洗嗎？","若您的冷氣位於挑高天花板、狹窄空間，或須使用特殊梯具才能作業的位置，請在預約時提前告知客服或於備註欄說明。技師將於現場評估可行性，可能需您協助提供高梯，或因作業難度較高而酌收額外費用。若評估後認定施工風險過高，為維護安全，將無法提供清洗服務。"],["冷氣清洗前需要先準備什麼？","現場需可提供基本水源、110V 電源與排水動線，並請先清空冷氣下方至少 1.5 x 1.5 公尺的範圍，方便技師架梯、防護與施工。"],["預約前需要拍哪些冷氣照片？（拍攝重點教學）","為利技師初步判斷現場是否可提供清洗服務，請依機型協助拍攝以下照片：\n\n① 壁掛式室內機\n・正面全景：拍到完整室內機\n・左右兩側空間：拍到冷氣與牆面、櫃體的距離\n・上方空間：拍到冷氣頂部與天花板距離\n・下方環境：拍到冷氣下方是否有床、櫃子、電視或其他大型家具\n・整體安裝位置：建議退後拍一張，讓技師看得出冷氣的實際位置\n\n② 吊隱式室內機\n・出風口／回風口全景\n・天花板整體位置：退後拍攝，讓技師看得出設備安裝位置\n・維修孔照片：需拍到完整維修孔及周邊空間\n・維修孔打開後內部：若方便開啟，請拍攝內部設備位置\n・下方環境：拍到設備下方是否有固定櫃體、家具或其他障礙物\n＊吊隱式特別重要：讓技師確認維修孔的位置、大小及是否有足夠拆洗空間。\n\n③ 四方吹室內機\n・室內機正下方全景：完整拍到四方吹面板\n・天花板整體位置：退後拍攝設備與周邊環境\n・設備四周空間：確認旁邊是否有燈具、消防設備、裝潢或其他阻礙\n・下方環境：拍到下方是否有桌椅、櫃體或固定設備\n・安裝高度：若位置較高，請盡量從遠處拍攝一張可判斷高度的照片\n\n＊拍攝小提醒：照片請盡量清楚、明亮、不裁切設備，並稍微退後拍攝，讓技師同時看到「空調＋周邊環境」。"],["壁掛式／吊隱式／四方吹空調室內機下方，需要預留多少施工空間？","為利技師進行清洗作業，建議室內機下方預留足夠施工範圍，以利：\n1. 梯具擺放\n2. 清洗設備操作\n3. 零件拆裝\n4. 拆下零件暫時放置\n\n建議預留空間如下：\n・一般建議至少預留約 150 × 150 公分的作業空間\n・壁掛式室內機：通常仍需保留下方基本施工空間\n・吊隱式室內機：因施工與拆裝較繁複，通常需預留更充足空間\n・四方吹室內機：因作業方式與設備配置需求，也建議保留較完整的施工範圍\n\n提醒您：若室內機下方空間不足，或周邊有大型家具、裝潢、隔間等影響施工之情況，可能會影響現場作業安排，實際仍須由技師依現場環境評估是否可施作。"],["吊隱式一定要有維修孔才能清洗嗎？若維修孔不夠大怎麼辦？","通常需有適當維修孔，方便技師拆裝與清洗內部零件。若維修孔過小、位置不佳或尺寸不足，將影響拆裝與清潔作業，現場可能改為部分清潔、加價處理，或無法施作。\n※ 四方吹機型同理，若天花板維修孔條件不佳，亦可能影響施工範圍。"],["家中有貴重物品或怕水家具，清洗時會不會弄濕弄髒？","施工前技師會進行防水遮蔽與現場保護，完工後整理環境。\n但仍建議事先將貴重物品、木質家具與怕水電器自行移開或妥善保護；若不方便移動，請事先告知以利技師加強防護。"],["大樓、社區、公寓施工前需要先做什麼？","若社區有施工申請、換證、梯控、時段限制或管委會規範，請務必提前確認，避免技師到場後無法進場施工。"],["天花板太高或無法安全架梯，還能施工嗎？","若安裝高度過高、現場因空間或地面條件限制導致無法安全架梯，或施工風險過高，技師會依安全原則評估，必要時可能需加價或婉拒服務。"],["若現場沒有水源或電源，還可以清洗嗎？","通常不行。現場需具備基本 110V 電源、水源與排水條件，若無法提供，技師可能無法執行清洗作業。"],["冷氣清洗前需要先關機嗎？","建議您在師傅到訪前，提前關閉冷氣約 30 分鐘至 1 小時。這樣可讓機體內部恢復至常溫，避免清洗時產生過多冷凝水，影響作業安全與清潔效果。"],["冷氣下方需要清空嗎？","是的，強烈建議您這麼做！ 為了保護您的傢俱與電器，並讓技師有足夠的空間安全地作業，請您在技師抵達前，清空冷氣下方至少 1.5 x 1.5 公尺的範圍。 特別是床、沙發、書桌、電視、電腦等貴重或怕水的物品，請務必移開或使用防水布妥善覆蓋喔！"],["寵物需要先隔離嗎？","是的，為了毛孩子的安全，也為了讓技師能專心服務，請務必將寵物（無論是貓、狗或鳥等）暫時隔離在另一個房間或籠內。 因為清洗過程中，除了設備運轉的聲響外，也可能會有些許清潔劑的味道，先將毛孩安置好，對牠們來說是比較安心的作法喔！"],["我需要全程在家嗎？","是的，為了保障您的權益，建議您或您的家人需要全程在家。 技師在服務過程中，可能會需要與您確認一些設備的細節狀況，或是在需要移動物品時取得您的同意。您在現場，我們也能更安心地進行最完善的服務。 此外，在服務結束後的最終功能驗收，也需要您一同在場確認，確保一切都回復到最佳狀態喔！"],["服務後會很髒亂嗎？","請您完全放心！ 我們的技師都受過專業訓練，在服務開始前，會使用專業的防水罩與養生膠帶，將冷氣周遭的牆面與傢俱做最完善的保護。 服務結束後，我們也會將工作區域清潔並恢復原狀，確保給您一個乾淨舒適的環境。"]]],["E","清洗效益、頻率與保養",[["清洗後是否一定會變冷、變省電？","清洗可改善因髒污造成的出風不順、異味、滴水或散熱效率下降等狀況；若冷氣本身已有冷媒不足、電控異常、零件老化等問題，仍需另行檢修。"],["冷氣的清潔頻率應該如何拿捏？","一般家庭：建議每年一次；若家中有寵物、過敏成員或近大馬路可增加頻率。\n商業場所：因運轉時間長，建議每半年一次。"],["定期清洗冷氣有哪些必要性？","能維持室內空氣品質、改善異味與過敏問題、提升冷房效率以節省電費。"],["冷氣清洗後建議多久再洗一次？","一般家庭建議每年清洗一次即可，能維持良好效能與空氣品質。若家中有過敏體質成員、飼養寵物，或冷氣使用頻繁（如商業空間、店鋪），建議將清洗週期縮短為每半年一次，確保運轉效率與衛生。"],["日常生活中有哪些保養冷氣的好習慣？","開關機前先送風 3–5 分鐘保持內部乾燥；使用期間濾網約兩週清洗一次；室外機周遭保持通風勿堆雜物；長時間停用前開送風運轉半天吹乾內部。"],["冷氣清洗後有什麼好處？真的能省電嗎？","清洗能有效去除冷氣內部的灰塵與黴菌，讓風量更順暢、冷房效率提升，同時減少異味與細菌滋生。由於機器運轉更輕鬆，確實有助於降低耗電量，還能延長冷氣的使用壽命。"],["多久預約一次清洗服務比較好？","一般家庭建議每年清洗一次。若家中有過敏體質者、嬰幼兒、長者，或有飼養寵物，以及冷氣使用頻率高（如營業場所），建議縮短至半年清洗一次，以維持空氣品質與機器效能。"],["清洗後要怎麼保養才能延長效果？","建議您每隔 2～4 週依使用頻率清洗或更換濾網，並在換季或長時間未使用前，開啟送風模式運轉約 10～15 分鐘，讓機體內部保持乾燥。這樣能有效抑制黴菌滋生，延續冷氣清洗後的潔淨與效能。"],["清洗後冷氣能恢復像新的一樣嗎？","專業清洗可明顯改善冷房效率、去除異味並提升空氣品質。雖然無法讓使用多年的冷氣機完全恢復如新（外觀或零件可能仍有自然老化痕跡），但內部清潔後可使運轉效能接近最佳狀態，並有效延長機器壽命。"],["除了請專業人員清洗外，平時我可以怎麼保養冷氣？","除了定期請專業技師清洗外，建議您每隔 2～4 週視使用頻率與環境狀況，清洗或更換濾網。另可在關機前切換至送風模式約 10～15 分鐘，幫助機體內部乾燥，降低潮濕與黴菌滋生的機率。"]]],["F","特殊情況與注意事項",[["若四方吹/吊隱式機型特殊、結構受限，是否仍可完整清洗？","不一定。部分特殊機型、裝潢封板過密、維修孔位置不佳或天花結構複雜者，可能僅能施作可及範圍，實際需由現場技師判斷。"],["冷氣清洗後，異味或霉味就一定會完全消失嗎？","若異味來源位於本次可清洗範圍內，例如蒸發器、貫流風扇、風箱、集水盤等部位，清洗後通常會有明顯改善。但若氣味來源並非來自機體可清洗區域，例如壁內排水管沼氣回流、裝潢夾層異味，或室內通風條件不佳等情況，則仍需進一步處理根本原因，才能較完整地改善異味問題。"],["清洗後若冷氣出現問題該怎麼處理？","自清洗服務完成日起，享有 30 天基本功能保固。\n\n【保固範圍】\n・保固期內若經原廠鑑定確認為清洗直接導致的功能異常，依設備機齡分級理賠\n・服務過程中因人員疏失造成傢俱損傷，釐清責任後最高賠償 NT$10,000\n\n【不提供保固的情況】\n・設備使用超過 10 年（零件老化風險高，恕不提供保固）\n・清洗前已存在的設備問題或舊有損壞\n・消耗性零配件自然損耗、設備自然磨損\n・天災、外力或使用者不當操作所致\n\n【申請流程】\n1. 保固期內發現異常，聯繫客服並提供照片或影片\n2. 指派原廠技術人員到府鑑定，釐清問題根源\n3. 確認為清洗所致：後續維修由原廠全權協調處理\n   非關清洗問題：相關維修費用由您負擔\n\n＊保固期無法因「未使用、出國」等個人因素延長，請在期限內多加使用測試。\n＊服務完成後請務必與技師一同驗收，以保障您的權益。\n＊以上為重點摘要，完整條款以賣場公告為準。"],["老舊機器是否也可以清洗？清洗後若出現零件脆化怎麼辦？","可先評估。\n但若機器年限較高，塑膠件、泡棉、保溫材、排水管等耗材可能已有老化現象，清洗過程中較容易顯現原有風險，並非一定屬施工異常。\n⚠️ 設備使用超過 10 年者，因零件老化風險高，清洗後恕不提供功能保固。\n⚠️ 設備使用超過 15 年，或超過 10 年且環境惡劣／整體狀況不佳者，恕不提供清洗服務。"],["排水管原本就不順，清洗後一定會改善嗎？","不一定。若排水管已有老化、坡度異常、嚴重堵塞或安裝問題，清洗只能協助基本保養，若有結構性問題仍需另行處理。"],["清洗後若有些微味道或潮濕感，正常嗎？需要做什麼？","清洗後短時間內若有些微濕氣、藥劑或水氣殘留味道，多屬正常現象。\n建議開窗通風並以送風、強風或除濕模式持續運轉 30～60 分鐘，有助於帶走殘留濕氣與味道。"],["濾網要從哪裡購買？","請洽詢當初購買三菱重工空調的經銷商或直接來電上洋客服專線0800-300766購買。"],["遙控器要從哪裡購買？","請洽詢當初購買三菱重工空調的經銷商或直接來電上洋客服專線0800-300766購買。"],["我的冷氣使用多年，還適合清洗嗎？","若冷氣已使用超過 10～15 年，因零件老化，清洗時確實可能增加拆裝或漏水等風險。\n技師會於現場檢視機況並評估可否安全清洗；若判定風險過高，會建議您先行維修或考量是否仍有清洗的必要。\n⚠️ 設備使用超過 10 年者，因零件老化風險高，清洗後恕不提供功能保固。\n⚠️ 設備使用超過 15 年，或超過 10 年且長期處於硫磺／鹽害環境、整體狀況不佳者，因故障風險過高，恕不提供清洗服務。"],["遇到颱風或豪大雨，原本預約的冷氣清洗服務還會到府嗎？","為保障服務人員安全及維持服務品質，若服務當日遇颱風或豪大雨影響，將依以下原則辦理：\n\n📍 1. 服務地區公告停止上班上課\n若您服務地址所在縣市政府已公告「停止上班上課」，原訂到府清洗服務將暫停，後續將由客服主動與您聯繫重新安排服務時間。\n\n📍 2. 未停班停課，但現場不適合前往\n若服務地區雖未公告停班停課，但服務人員依現場風勢、雨勢、道路狀況或其他安全因素評估無法安全前往，亦將主動與您聯繫改期。"]]]];
function _qdText(el,txt){var re=/https?:\/\/[^\s）)」]+/g,last=0,m;while((m=re.exec(txt))){if(m.index>last)el.appendChild(document.createTextNode(txt.slice(last,m.index)));var a=document.createElement('a');a.href=m[0];a.target='_blank';a.rel='noopener';a.textContent=m[0];el.appendChild(a);last=m.index+m[0].length;}if(last<txt.length)el.appendChild(document.createTextNode(txt.slice(last)));}
function _qdFaq(b,imgs){try{
  if(!QD_FAQ||!QD_FAQ.length)return;
  var box=document.createElement('div');box.className='qd-faq';
  QD_FAQ.forEach(function(c){
    var hd=document.createElement('div');hd.className='qd-fk';
    var k=document.createElement('b');k.textContent=c[0];var t=document.createElement('span');t.textContent=c[1];
    hd.appendChild(k);hd.appendChild(t);box.appendChild(hd);
    c[2].forEach(function(it){
      var d=document.createElement('details');var sm=document.createElement('summary');
      var q=document.createElement('span');q.className='qd-q';q.textContent='Q';
      var qt=document.createElement('span');qt.textContent=it[0];
      var ar=document.createElement('span');ar.className='qd-ar';ar.textContent='▾';
      sm.appendChild(q);sm.appendChild(qt);sm.appendChild(ar);d.appendChild(sm);
      var an=document.createElement('div');an.className='qd-a';_qdText(an,it[1]);d.appendChild(an);
      box.appendChild(d);
    });
  });
  b.appendChild(box);
  imgs.forEach(function(i){i.style.display='none';});/* 文字版建好才藏圖(藏起來的延遲載入圖不會下載);任何錯誤就保留原圖 */
}catch(e){}}
var QD_GROUPS=[
 ['認識清洗','🧼',['02','03'],1],
 ['適用機型與清洗須知','📋',['04','05','06','07'],1],
 ['方案與價格','💰',['09','10','11','12'],1],
 ['預約與到府流程','📅',['13','14','15','16'],1],
 ['常見問題','❓',['17','18','19'],1],
 ['聯繫我們','📞',['20'],1]
];
...
var QD_CSS='.qd-toc{position:sticky;top:0;z-index:50;background:#fff;border-bottom:1px solid #e3e8ef;padding:10px 12px;display:flex;gap:8px;overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch}'
      ...
      +'.qd-faq{padding:4px 16px 24px;background:#fff;text-align:left}'
      +'.qd-fk{display:flex;align-items:baseline;gap:10px;margin:24px 0 4px;padding-bottom:8px;border-bottom:2px solid #1d2a3a;color:#1d2a3a;font-size:16px;font-weight:700}'
      +'.qd-fk b{color:#b3202a;font-size:28px;line-height:1;font-family:Georgia,serif}'
      +'.qd-faq details{border-bottom:1px solid #e3e8ef}'
      +'.qd-faq summary{list-style:none;cursor:pointer;display:flex;gap:10px;align-items:flex-start;padding:14px 0;font-size:15px;font-weight:600;line-height:1.5;color:#1d2a3a;-webkit-tap-highlight-color:transparent}'
      +'.qd-faq summary::-webkit-details-marker{display:none}'
      +'.qd-q{flex:none;display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;margin-top:2px;border-radius:3px;background:#1d2a3a;color:#fff;font-size:11px;font-weight:700}'
      +'.qd-ar{flex:none;margin-left:auto;color:#8a6a1f;transition:transform .2s}'
      +'.qd-faq details[open] .qd-ar{transform:rotate(180deg)}'
      +'.qd-a{padding:0 0 16px 30px;font-size:14px;line-height:1.8;color:#3a4656;white-space:pre-line;word-break:break-word}'
      +'.qd-a a{color:#123a66;text-decoration:underline}'
      +'@media (prefers-reduced-motion:reduce){.qd-ar{transition:none}}';
/* 2026-09-22 詳情頁 HTML 文字版:內容放在 GitHub m3 的 detail/(detail.html + img/*.webp),跟 wizard.js 同一個版本號。
   載入成功 → 用「隔離盒子」(Shadow DOM)顯示,詳情頁的樣式不會跟 1SHOP 頁面互相干擾;原本 19 張詳情圖藏起來(不下載)。
   載入失敗或 6 秒沒回應 → 自動改用原本的圖片版,目錄/收展照常。 */
function _qdBase(){try{var ss=document.querySelectorAll('script[src*="/m3@"]');for(var i=0;i<ss.length;i++){var m=(ss[i].getAttribute('src')||'').match(/^(.*\/m3@[^\/]+\/)wizard\.js/);if(m)return m[1]+'detail/';}}catch(e){}return null;}
function _qdHead(g,gi,heads){
...
function _qdToc(heads,before){
  if(!heads.length)return;
  var toc=document.createElement('nav');toc.className='qd-toc';toc.setAttribute('aria-label','詳情目錄');
  heads.forEach(function(x){toc.appendChild(x[1]);});
  before.parentElement.insertBefore(toc,before);
...
function _qdImgVersion(imgs,box){try{
  var heads=[];
  QD_GROUPS.forEach(function(g,gi){
    var mine=imgs.filter(function(i){return _qdOf(i.getAttribute('alt')||'',g[2]);});
    if(!mine.length)return;
    var hb=_qdHead(g,gi,heads);
    box.insertBefore(hb.h,mine[0]);box.insertBefore(hb.b,mine[0]);
    mine.forEach(function(i){hb.b.appendChild(i);});
    if(g[0]==='常見問題')_qdFaq(hb.b,mine);
  });
  _qdToc(heads,box);
}catch(e){}}
/* 服務區域表:原本 HTML 裡的小程式(依 CSV 產生表格),搬到這裡在隔離盒子裡執行 */
function _qdArea(sr,csv){try{
  var tb=sr.querySelector('#serviceAreaTable tbody');if(!tb||!csv)return;
  var rows=csv.trim().split('\n').slice(1),g={},order=[],cur='';
  rows.forEach(function(row){var c=row.split(',');var k=(c[0]||'').trim();if(k){cur=k;if(!g[cur]){g[cur]=[[],[],[]];order.push(cur);}}if(!cur)return;for(var i=1;i<=3;i++){if(c[i]&&c[i].trim())g[cur][i-1].push(c[i].trim());}});
  function li(items,cls){if(!items.length)return '<li style="color:#aaa;font-size:12px;">—</li>';return items.map(function(x){return '<li class="'+cls+'">'+x+'</li>';}).join('');}
  order.forEach(function(k){var tr=document.createElement('tr');tr.innerHTML='<td class="county-cell">'+k+'</td><td><ul class="area-list">'+li(g[k][0],'area-available')+'</ul></td><td><ul class="area-list">'+li(g[k][1],'area-remote')+'</ul></td><td><ul class="area-list">'+li(g[k][2],'area-unavailable')+'</ul></td>';tb.appendChild(tr);});
}catch(e){}}
function _qdHtmlVersion(t,base,box){try{
  var host=document.createElement('div');host.className='qd-host';
  if(!host.attachShadow)return false;
  var sr=host.attachShadow({mode:'open'});
  sr.innerHTML='<style>'+QD_CSS+'</style>'+t.split('__B__').join(base);
  var root=sr.querySelector('.qdd');if(!root)return false;
  var secs=[].slice.call(root.children).filter(function(x){return x.hasAttribute('data-qg');});
  if(secs.length<10)return false;
  var csvEl=sr.getElementById('qd-area-csv');_qdArea(sr,csvEl?csvEl.textContent:'');
  var heads=[];
  QD_GROUPS.forEach(function(g,gi){
    var mine=secs.filter(function(x){return x.getAttribute('data-qg')===String(gi);});
    if(!mine.length)return;
    var hb=_qdHead(g,gi,heads);
    root.insertBefore(hb.h,mine[0]);root.insertBefore(hb.b,mine[0]);
    mine.forEach(function(x){hb.b.appendChild(x);});
    if(g[0]==='常見問題'){var fq=hb.b.querySelector('[data-qfaq]');_qdFaq(fq||hb.b,[]);}
  });
  box.parentElement.insertBefore(host,box);
  box.style.display='none';/* 原圖藏起來:延遲載入的圖不會再下載 */
  _qdToc(heads,host);
  window.__qdHtml=1;
  return true;
}catch(e){return false;}}
...
function buildDetailToc(){try{
  if(_qdStarted||document.querySelector('.qd-toc'))return true;
  ...
  _qdStarted=1;
  if(!document.getElementById('qd-css')){var st=document.createElement('style');st.id='qd-css';st.textContent=QD_CSS;document.head.appendChild(st);}
  var base=_qdBase();
  if(!base||!window.fetch||!window.Promise){_qdImgVersion(imgs,box);return true;}
  var done=false;
  var to=setTimeout(function(){if(done)return;done=true;_qdImgVersion(imgs,box);},6000);
  fetch(base+'detail.html').then(function(r){if(!r.ok)throw 0;return r.text();}).then(function(t){
    if(done)return;done=true;clearTimeout(to);
    if(!_qdHtmlVersion(t,base,box))_qdImgVersion(imgs,box);
  })['catch'](function(){if(done)return;done=true;clearTimeout(to);_qdImgVersion(imgs,box);});
  return true;
}catch(e){return true;}}
